...
<tr>
	...
	&nbsp;	
	<%}else{ %>			
		<a href = "qwriteForm.jsp">글쓰기</a>&nbsp;
	<% } %>	
	</td>
</tr>	
</table>
<%     //boardq 테이블에 저장된 레코드가 없으면
	if(count == 0)
	{
%>
...
<%		//글 목록을 한 줄씩 화면에 표시하는 부분. articleList.size()는 글 목록의 사이즈를 구하여 반복.
		...
		{
			BoardqDataBean article = (BoardqDataBean)articleList.get(i);
%>
<tr height = "30">
...
%>		<!--  글을 목록에 한 줄씩 표시한다. 글의 제목을 content.jsp로 이동하여 글의 내용을 볼 수 있도록 링크를 걸어줌 -->
		<a href = "qcontent.jsp?num=<%= article.getNum() %>&pageNum=<%= currentPage %>">
			<%= article.getSubject() %>
		</a>
<%
...
<%
	if(count > 0)
	{
		...
		{
%>
			<a href = "qlist.jsp?pageNum=<%= startPage-5 %>">[이전]</a>
<%
		}
		...
		{
%>
			<a href = "qlist.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%
		}
		...
		{
%>
			<a href = "qlist.jsp?pageNum=<%= startPage+5 %>">[다음]</a>
<%
		}
	}
%>
